...
- 以 Scopus Search API 完成两阶段文献检索（宽泛 → 聚焦），并比较两轮结果之差异
...
- 操作：
  ...
  4. 提交后，页面显示一串字母数字组合之 API key（形如 `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`）——复制保存，步骤 3 使用
...
- 操作：
  ...
  2. 执行该单元——笔记本以 Scopus Search API 检索，输出前 10 笔结果表格（标题、年份、DOI、被引次数）
  3. 阅读标题，凭标题选出与你主题最相关的 5 篇
- 预期结果：表格显示 10 列检索结果
- 说明：查询语法 `TITLE-ABS-KEY(...)` 同时检索标题、摘要与关键词栏位；`PUBYEAR > 2022` 限 2023 年以来之文献
- 失败处理：0 笔结果 → 主题词过窄——减少关键词或改用更广泛之同义词后重跑此单元；显示 401／未授权 → 回步骤 1、3 检查 key；显示配额（quota）提示 → 稍候数分钟再试

### 步骤 5：第二轮检索（聚焦）

- 操作：
  1. 从步骤 4 所选 5 篇之标题（需要时于 Scopus 网页查看摘要）归纳 1–2 组更精确之关键词组合
...
- 失败处理：查询显示 422 错误 → `CONTACT_EMAIL` 须为真实格式之邮箱（回步骤 3 修正）；个别论文下载失败 → 跳过并记录，不影响其余篇数
