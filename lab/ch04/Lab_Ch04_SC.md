...
- 说明：两阶段检索为研究工作之标准起手式——宽泛检索掌握全局，聚焦检索逼近研究问题；完整研究流程中，本步骤之结果（子议题归纳与聚焦检索式）应以 AI 代理（agentic AI）产出（后续课程主题）——本单元尚未涵盖 AI 代理操作；notebook 中聚焦检索式与子议题记录为硬编码（hardcoded）范例（仅示范输出格式），实际操作请依你的检索结果改写
